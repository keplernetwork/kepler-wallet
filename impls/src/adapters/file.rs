	...
	fn put_tx(&self, slate: &Slate) -> Result<(), Error> {
		...
		let out_slate = {
			if slate.payment_proof.is_some() || slate.ttl_cutoff_height.is_some() {
				warn!("Transaction contains features that require kepler-wallet 3.0.0 or later");
				warn!("Please ensure the other party is running kepler-wallet v3.0.0 or later before sending");
