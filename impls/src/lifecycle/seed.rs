...
impl WalletSeed {
	...
	pub fn recover_from_phrase(
		...
		if !Path::new(&data_file_dir).exists() {
			return Err(ErrorKind::WalletDoesntExist(
				...
				"To create a new wallet from a recovery phrase, use 'kepler-wallet init -r'"
					.to_owned(),
			))?;
		}
	...
	pub fn from_file(
		...
		if Path::new(seed_file_path).exists() {
			...
			error!(
				"wallet seed file {} could not be opened (kepler wallet init). \
				 Run \"kepler wallet init\" to initialize a new wallet.",
				seed_file_path
			);
