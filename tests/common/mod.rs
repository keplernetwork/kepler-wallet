...
use kepler_wallet_impls::test_framework::LocalWalletClient;
...
use kepler_wallet_api::{EncryptedRequest, EncryptedResponse};
...
use kepler_wallet_libwallet::{NodeClient, WalletInfo, WalletInst};
use kepler_wallet_util::kepler_core::global::{self, ChainTypes};
use kepler_wallet_util::kepler_keychain::ExtKeychain;
use kepler_wallet_util::kepler_util::{from_hex, static_secp_instance};
use util::secp::key::{PublicKey, SecretKey};

use kepler_wallet::cmd::wallet_args;
...
macro_rules! setup_proxy {
	($test_dir: expr, $chain: ident, $wallet1: ident, $client1: ident, $mask1: ident, $wallet2: ident, $client2: ident, $mask2: ident) => {
		...
		let target = std::path::PathBuf::from(format!("{}/wallet1/kepler-wallet.toml", $test_dir));
		println!("{:?}", target);
		if !target.exists() {
		...
		let target = std::path::PathBuf::from(format!("{}/wallet2/kepler-wallet.toml", $test_dir));
		if !target.exists() {
