...
use kepler_wallet_api::ECDHPubkey;
...
use kepler_wallet_impls::DefaultLCProvider;
use kepler_wallet_libwallet::{InitTxArgs, Slate, SlateVersion, VersionedSlate};
use kepler_wallet_util::kepler_keychain::ExtKeychain;
use serde_json;

use kepler_wallet_util::kepler_util::Mutex;
...
#[test]
fn owner_v3_lifecycle() -> Result<(), kepler_wallet_controller::Error> {
	...
	let (chain, wallet2, mask2_i) = {
		...
		let arg_vec = vec!["kepler-wallet", "owner_api", "-l", "43420", "--run_foreign"];
	...
	// 2) get the top level directory, should default to ~/.kepler/auto
	...
	println!("RES 4: {:?}", res);
	assert!(res.is_ok());
	let pb = PathBuf::from(format!("{}/wallet1/kepler-wallet.toml", test_dir));
	assert!(pb.exists());
	...
	//15) Ask wallet 2 for some keplers
	let req = serde_json::json!({
		"jsonrpc": "2.0",
		"id": 1,
		...
		"params": {
			...
			"args": {
				"amount": "6000000000",
				"message": "geez a block of keplers",
				"dest_acct_name": null,
	...
	kepler_wallet_controller::controller::owner_single_use(wallet2.clone(), mask2, |api, m| {
		let args = InitTxArgs {
			src_acct_name: None,
			amount: slate.amount,
			minimum_confirmations: 1,
