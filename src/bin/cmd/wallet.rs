...
use kepler_wallet_libwallet::NodeClient;
...
pub fn wallet_command(wallet_args: &ArgMatches<'_>, config: GlobalWalletConfig) -> i32 {
	...
	match node_client.clone().chain_height() {
		Ok(h) => {
			if h >= 262080 {
				let err_str = "This version of kepler-wallet is obsolete as of block 262080. Please download v2.0.0 from https://github.com/keplernetwork/kepler-wallet/releases";
				error!("{}", err_str);
				println!();
				println!("***************");
				println!("{}", err_str);
