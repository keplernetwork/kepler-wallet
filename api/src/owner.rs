	...
	K: Keychain,
{
	...
	/// `kepler-wallet.toml` file and the `wallet_data` directory that contains the wallet
	...
	///
	/// 1) The current directory, from which `kepler-wallet` or the main process was run, if it
	/// contains a `kepler-wallet.toml` file.
	/// 2) ~/.kepler/<chaintype>/ otherwise
	///
	/// # Arguments
	///
	/// * None
	...
	/// Create a `kepler-wallet.toml` configuration file in the top-level directory for the
	/// specified chain type.
	/// A custom [`WalletConfig`](../kepler_wallet_config/types/struct.WalletConfig.html)
	/// and/or kepler `LoggingConfig` may optionally be provided, otherwise defaults will be used.
	///
	...
	/// path-related values in the optional configuration structs will be ignored.
	///
	/// The wallet files must not already exist, and ~The `kepler-wallet.toml` file must exist
	...
	///
	/// # Returns
	/// * Ok with a Vec of [`StatusMessage`](../kepler_wallet_libwallet/api_impl/owner_updater/enum.StatusMessage.html)
	/// * or [`libwallet::Error`](../kepler_wallet_libwallet/struct.Error.html) if an error is encountered.
	///
	/// # Example
	/// Set up as in [`new`](struct.Owner.html#method.new) method above.
