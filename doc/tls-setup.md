...
* DNS configuration for this IP (`kepler1.example.com` -> `3.3.3.3`)
...
Uncomment and update the following lines in wallet config (by default `~/.kepler/kepler-wallet.toml`):
...
```sh
sudo certbot certonly --standalone -d kepler1.example.com
```
...
Congratulations! Your certificate and chain have been saved at:
  /etc/letsencrypt/live/kepler1.example.com/fullchain.pem
 Your key file has been saved at:
  /etc/letsencrypt/live/kepler1.example.com/privkey.pem
...
### Change permissions
Now you have the certificate files but only root user can read it. We run kepler as `ubuntu` user. There are different scenarios how to fix it, the simplest one is to create a group which will have access to `/etc/letsencrypt` directory and add our user to this group.
...
```toml
tls_certificate_file = "/etc/letsencrypt/live/kepler1.example.com/fullchain.pem"
tls_certificate_key =  "/etc/letsencrypt/live/kepler1.example.com/privkey.pem"
```
